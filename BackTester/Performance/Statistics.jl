...
function outputbackteststatistics_partial(accttrkr::AccountTracker,
    ...
    for i = 1:length(sorteddates)
        equity[string(sorteddates[i])] = round(pftrkr[sorteddates[i]].portfoliostats.netvalue, digits = 2)
        totalreturn_algorithm[string(sorteddates[i])] = round(100.0 * pftrkr[sorteddates[i]].returns.totalreturn, digits = 2)
        totalreturn_benchmark[string(sorteddates[i])] = round(100.0 * bnchtrkr[sorteddates[i]].returns.totalreturn, digits = 2)
    end
...
function outputbackteststatistics_full(accttrkr::AccountTracker,
    ...
    outputdict["tradebook"] = serialize(trdbk)

    tradestats = computeAllTradeStats(trdbk, date)

    outputdict["tradestats"] = serialize(tradestats)
...
function getaggregatereturns(pft::PerformanceTracker, symbol::Symbol = :All)
    ...
    while i < length(sorteddates)
        ...
        if (nyear == year)
            ...
            yearlyreturns[ystr] = round((yret - 1.0)*100.0, digits = 2)
            yret = (1.0 + dailyreturn)
        end
        ...
        if (nmonth == month)
            ...
            monthlyreturns[mstr] = round((mret - 1.0)*100.0, digits = 2)
            mret = (1.0 + dailyreturn)
        end
        ...
        if (nweek == week)
            ...
            weeklyreturns[wstr] = round((wret - 1.0)*100.0, digits = 2)
            wret = (1.0 + dailyreturn)
        end
...
function convert(::Type{Dict}, performance::Performance)
    Dict{String, Any}(  "annualreturn" => round(100.0 * performance.returns.annualreturn, digits = 2),
                        "totalreturn" => round(100.0 * performance.returns.totalreturn, digits = 2),
                        "annualstandarddeviation" => round(100.0 * performance.deviation.annualstandarddeviation, digits = 2),
                        #"annualvariance" => round(100.0 * 100.0 * performance.annualvariance,2),
                        "sharperatio" => round(performance.ratios.sharperatio, digits = 2),
                        "informationratio" => round(performance.ratios.informationratio, digits = 2),
                        #"drawdown" => round(100.0 * performance.drawdown.currentdrawdown, 2),
                        "maxdrawdown" => round(100.0 * performance.drawdown.maxdrawdown, digits = 2),
                        "period" => performance.period,
                        #"sortinoratio" => round(performance.ratios.sortinoratio, 2),
                        "calmarratio" => round(performance.ratios.calmarratio, digits = 2),
                        "stability" => round(performance.ratios.stability, digits = 2),
                        "beta" => round(performance.ratios.beta, digits = 2),
                        "alpha" => round(100*252*performance.ratios.alpha, digits = 2),
                    )
end
...
function outputperformanceJSON(performancetracker::PerformanceTracker, benchmarktracker::PerformanceTracker, variabletracker::VariableTracker, date::Date)
    ...
    jsondict = Dict{String, Any}("outputtype" => "performance",
                                "date" => date,
                                "dailyreturn" => round(100.0*performance.returns.dailyreturn, digits = 2),
                                "netvalue" => round(performance.portfoliostats.netvalue, digits = 2),
                                "annualreturn" => round(100.0*performance.returns.annualreturn, digits = 2),
                                "totalreturn" => round(100.0*performance.returns.totalreturn, digits = 2),
                                "totalreturn_benchmark" => round(100.0*btperformance.returns.totalreturn, digits = 2),
                                "annualstandarddeviation" => round(100.0*performance.deviation.annualstandarddeviation, digits = 2),
                                #"annualvariance" => round(100.0*100.0*performance.annualvariance,2),
                                "sharperatio" => round(performance.ratios.sharperatio, digits = 2),
                                "informationratio" => round(performance.ratios.informationratio, digits = 2),
                                ...
                                "stability" => round(performance.ratios.stability, digits = 2),
                                "beta" => round(performance.ratios.beta, digits = 2),
                                "alpha" => round(252*100*performance.ratios.alpha, digits = 2),
                                "drawdown" => round(100.0*performance.drawdown.currentdrawdown, digits = 2),
                                "maxdrawdown" => round(100.0*performance.drawdown.maxdrawdown, digits = 2),
                                "leverage" => round(performance.portfoliostats.leverage, digits = 2)
                            )
    if haskey(variabletracker, date)
        jsondict["variables"] = Dict{String, Float64}()

        for (k,v) in variabletracker[date]
            jsondict["variables"][k] = round(v, digits = 2)
        end
    end
