...
function parseline(l::AbstractString, s::TCPSocket)
    reply_mutable struct = l[1]
    reply_token = l[2:end]
    if reply_mutable struct == '+'
        return reply_token
    elseif reply_mutable struct == ':'
...
immutable SubscriptionMessage
    ...
    function SubscriptionMessage(reply::AbstractArray)
        notification = reply
        message_mutable struct = notification[1]
        if message_mutable struct == "message"
            new(SubscriptionMessageType.Message, notification[2], notification[3])
        elseif message_mutable struct == "pmessage"
